...
fn mov_reg8_imm8_high() {
    ...
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.high(), 0xAB);
}
...
fn mov_bx_imm16() {
    ...
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.bx.word(), 0xBEEF);
}
...
fn mov_reg_reg_byte() {
    ...
    exec_n(&mut vm, 2);
    assert_eq!(vm.cpu.registers.cx.low(), 0x55);
}
...
fn mov_mem_reg_word() {
    ...
    exec_n(&mut vm, 2);
    assert_eq!(vm.cpu.registers.ds.read_word(0x100), 0xABCD);
}
...
fn mov_al_moffs() {
    ...
    vm.cpu.registers.ds.write_byte(0x200, 0x77);
    exec(&mut vm);
...
fn mov_ax_moffs() {
    ...
    vm.cpu.registers.ds.write_word(0x200, 0xBEEF);
    exec(&mut vm);
...
fn mov_moffs_al() {
    ...
    exec_n(&mut vm, 2);
    assert_eq!(vm.cpu.registers.ds.read_byte(0x300), 0x33);
}
...
fn mov_rm_imm_word() {
    ...
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ds.read_word(0x100), 0x5678);
}
...
fn mov_rm_imm_byte() {
    ...
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ds.read_byte(0x100), 0xAA);
}
...
fn mov_sreg_rm() {
    ...
    exec_n(&mut vm, 2);
    assert_eq!(vm.cpu.registers.es.reg().word(), 0x1000);
}
...
fn mov_rm_sreg() {
    ...
    vm.cpu.registers.ds.reg_mut().set(0x2000);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x2000);
}
...
fn xchg_rm_reg_word() {
    ...
    exec_n(&mut vm, 3);
    assert_eq!(vm.cpu.registers.ax.word(), 0xBBBB);
    assert_eq!(vm.cpu.registers.bx.word(), 0xAAAA);
}
...
fn lea_basic() {
    ...
    vm.cpu.registers.bx.set(0x100);
    vm.cpu.registers.si.set(0x050);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x150);
}
...
fn lds_basic() {
    ...
    vm.cpu.registers.ds.write_word(0x200, 0x1234);
    vm.cpu.registers.ds.write_word(0x202, 0x5678);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x1234);
    assert_eq!(vm.cpu.registers.ds.reg().word(), 0x5678);
}
...
fn les_basic() {
    ...
    vm.cpu.registers.ds.write_word(0x200, 0xAAAA);
    vm.cpu.registers.ds.write_word(0x202, 0xBBBB);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.bx.word(), 0xAAAA);
    assert_eq!(vm.cpu.registers.es.reg().word(), 0xBBBB);
}
...
fn push_pop_reg() {
    ...
    exec_n(&mut vm, 4);
    assert_eq!(vm.cpu.registers.bx.word(), 0x4321);
...
fn push_pop_sreg() {
    ...
    vm.cpu.registers.ds.reg_mut().set(0x3000);
    exec_n(&mut vm, 2);
    assert_eq!(vm.cpu.registers.es.reg().word(), 0x3000);
}
...
fn pushf_popf() {
    ...
    vm.cpu.flags = 0x0246; // ZF+PF+reserved
    exec(&mut vm); // PUSHF
    vm.cpu.flags = 0;
    exec(&mut vm); // POPF
    // 8086 reserved bits: bits 12-15 always 1, bit 1 always 1
    assert_eq!(vm.cpu.flags, 0xF246);
}

#[test]
fn lahf_sahf() {
    let mut vm = setup(&[0x9F, 0x9E]); // LAHF; SAHF
    vm.cpu.flags = 0x00D7; // SF+ZF+AF+PF+CF
    exec(&mut vm); // LAHF
    assert_eq!(vm.cpu.registers.ax.high(), 0xD7);

    // Now clear flags and restore from AH
    vm.cpu.flags = 0xFF00;
    exec(&mut vm); // SAHF
    assert_eq!(vm.cpu.flags & 0xFF, 0xD7);
}
...
fn xlat_basic() {
    ...
    vm.cpu.registers.bx.set(0x200);
    vm.cpu.registers.ax.set_low(5);
    vm.cpu.registers.ds.write_byte(0x205, 0x99);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x99);
}
...
fn cbw_positive() {
    ...
    vm.cpu.registers.ax.set_low(0x7F);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x007F);
}

#[test]
fn cbw_negative() {
    let mut vm = setup(&[0x98]);
    vm.cpu.registers.ax.set_low(0x80);
    exec(&mut vm);
...
fn cwd_positive() {
    ...
    vm.cpu.registers.ax.set(0x7FFF);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.dx.word(), 0x0000);
}

#[test]
fn cwd_negative() {
    let mut vm = setup(&[0x99]);
    vm.cpu.registers.ax.set(0x8000);
    exec(&mut vm);
...
fn add_al_imm() {
    ...
    vm.cpu.registers.ax.set_low(0x20);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x30);
    assert!(!vm.cpu.check_flag(Zero));
    assert!(!vm.cpu.check_flag(Carry));
    assert!(!vm.cpu.check_flag(Overflow));
}
...
fn add_ax_imm() {
    ...
    vm.cpu.registers.ax.set(0x2000);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x3000);
}
...
fn add_carry() {
    ...
    let mut vm = setup(&[0x04, 0x01]);
    vm.cpu.registers.ax.set_low(0xFF);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x00);
    assert!(vm.cpu.check_flag(Carry));
    assert!(vm.cpu.check_flag(Zero));
}
...
fn add_reg_reg() {
    ...
    exec_n(&mut vm, 3);
    assert_eq!(vm.cpu.registers.ax.word(), 8);
}
...
fn add_parity_flag() {
    ...
    vm.cpu.registers.ax.set_low(0x01);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x03);
    assert!(vm.cpu.check_flag(Parity)); // even number of 1-bits
}
...
fn adc_with_carry() {
    ...
    vm.cpu.registers.ax.set_low(0x10);
    vm.cpu.set_flag(Carry);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x12);
}
...
fn adc_without_carry() {
    ...
    vm.cpu.registers.ax.set_low(0x10);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x11);
}
...
fn adc_ax_imm_with_carry() {
    ...
    vm.cpu.registers.ax.set(0xFFFF);
    vm.cpu.set_flag(Carry);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x0001);
    assert!(vm.cpu.check_flag(Carry));
}
...
fn sub_al_imm() {
    ...
    vm.cpu.registers.ax.set_low(0x10);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x0B);
    assert!(!vm.cpu.check_flag(Carry));
    assert!(!vm.cpu.check_flag(Zero));
}
...
fn sub_borrow() {
    ...
    vm.cpu.registers.ax.set_low(0x05);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0xF5);
    assert!(vm.cpu.check_flag(Carry));
    assert!(vm.cpu.check_flag(Sign));
}
...
fn sub_zero() {
    ...
    assert!(vm.cpu.check_flag(Zero));
    assert!(parity(0));
    assert!(vm.cpu.check_flag(Parity));
}
...
fn sbb_al_with_carry() {
    ...
    vm.cpu.set_flag(Carry);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x0E);
}
...
fn cmp_equal() {
    ...
    vm.cpu.registers.ax.set_low(0x42);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x42); // unchanged
    assert!(vm.cpu.check_flag(Zero));
    assert!(!vm.cpu.check_flag(Carry));
}
...
fn cmp_less() {
    ...
    vm.cpu.registers.ax.set_low(0x10);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x10); // unchanged
    assert!(vm.cpu.check_flag(Carry));
    assert!(!vm.cpu.check_flag(Zero));
}
...
fn cmp_ax_imm() {
    ...
    vm.cpu.registers.ax.set(0x1000);
    exec(&mut vm);
    assert!(vm.cpu.check_flag(Zero));
}
...
fn inc_reg16() {
    ...
    vm.cpu.registers.ax.set(0x00FF);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x0100);
    assert!(!vm.cpu.check_flag(Zero));
}
...
fn inc_preserves_carry() {
    ...
    let mut vm = setup(&[0x40]);
    vm.cpu.registers.ax.set(0);
    vm.cpu.set_flag(Carry);
    exec(&mut vm);
    assert!(vm.cpu.check_flag(Carry)); // CF preserved
...
fn dec_reg16() {
    ...
    vm.cpu.registers.cx.set(0x0100);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.cx.word(), 0x00FF);
}

#[test]
fn dec_to_zero() {
    let mut vm = setup(&[0x49]);
    vm.cpu.registers.cx.set(1);
    exec(&mut vm);
...
fn neg_byte() {
    ...
    vm.cpu.registers.ax.set_low(0x05);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0xFB); // -5 = 0xFB
    assert!(vm.cpu.check_flag(Carry)); // CF=1 when operand != 0
    assert!(vm.cpu.check_flag(Sign));
}
...
fn neg_zero() {
    let mut vm = setup(&[0xF6, 0xD8]);
    vm.cpu.registers.ax.set_low(0x00);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x00);
    assert!(!vm.cpu.check_flag(Carry)); // CF=0 when operand == 0
    assert!(vm.cpu.check_flag(Zero));
}
...
fn mul_byte() {
    ...
    vm.cpu.registers.ax.set_low(10);
    vm.cpu.registers.cx.set_low(20);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 200);
    assert!(!vm.cpu.check_flag(Carry)); // high byte is 0
    assert!(!vm.cpu.check_flag(Overflow));
}
...
fn mul_byte_overflow() {
    ...
    let mut vm = setup(&[0xF6, 0xE1]);
    vm.cpu.registers.ax.set_low(0xFF);
    vm.cpu.registers.cx.set_low(0xFF);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0xFE01);
    assert!(vm.cpu.check_flag(Carry));
    assert!(vm.cpu.check_flag(Overflow));
}
...
fn mul_word() {
    ...
    vm.cpu.registers.ax.set(0x100);
    vm.cpu.registers.bx.set(0x100);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x0000);
    assert_eq!(vm.cpu.registers.dx.word(), 0x0001); // 0x10000
}
...
fn imul_byte() {
    ...
    vm.cpu.registers.ax.set_low(0xFE); // -2
    vm.cpu.registers.cx.set_low(3);
    exec(&mut vm);
...
fn imul_word() {
    ...
    vm.cpu.registers.ax.set(0xFFFE); // -2
    vm.cpu.registers.bx.set(0x0003); // 3
    exec(&mut vm);
    // -2 * 3 = -6 = 0xFFFF_FFFA -> DX=0xFFFF, AX=0xFFFA
    assert_eq!(vm.cpu.registers.ax.word(), 0xFFFA);
    assert_eq!(vm.cpu.registers.dx.word(), 0xFFFF);
}
...
fn div_byte() {
    ...
    vm.cpu.registers.ax.set(17);
    vm.cpu.registers.cx.set_low(5);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 3);  // 17/5 = 3
    assert_eq!(vm.cpu.registers.ax.high(), 2); // 17%5 = 2
}
...
fn div_word() {
    ...
    vm.cpu.registers.ax.set(100);
    vm.cpu.registers.bx.set(7);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 14); // 100/7 = 14
    assert_eq!(vm.cpu.registers.dx.word(), 2);  // 100%7 = 2
}
...
fn idiv_byte() {
    ...
    vm.cpu.registers.ax.set((-17i16) as u16); // 0xFFEF
    vm.cpu.registers.cx.set_low(5);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low() as i8, -3); // -17/5 = -3
    assert_eq!(vm.cpu.registers.ax.high() as i8, -2); // -17%5 = -2
}
...
fn idiv_word() {
    ...
    vm.cpu.registers.ax.set((-100i16) as u16);
    vm.cpu.registers.bx.set(7);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word() as i16, -14);
    assert_eq!(vm.cpu.registers.dx.word() as i16, -2);
}
...
fn group_83_add_sign_extend() {
    ...
    vm.cpu.registers.ax.set(0x0005);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x0004);
}
...
fn group_83_cmp() {
    ...
    exec(&mut vm);
    assert!(vm.cpu.check_flag(Zero));
    assert_eq!(vm.cpu.registers.ax.word(), 0x0005); // unchanged
}
...
fn group_80_and_byte() {
    ...
    vm.cpu.registers.ax.set_low(0xAB);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x0B);
    assert!(!vm.cpu.check_flag(Carry));
    assert!(!vm.cpu.check_flag(Overflow));
...
fn aaa_adjust() {
    ...
    vm.cpu.registers.ax.set_low(0x0F); // nibble >= 0xA
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x05); // (0x0F + 6) & 0x0F = 0x05
    assert!(vm.cpu.check_flag(Carry));
    assert!(vm.cpu.check_flag(AuxCarry));
}
...
fn aas_adjust() {
    ...
    vm.cpu.registers.ax.set_low(0x0F);
    exec(&mut vm);
    // AL = (0x0F - 6) & 0x0F = 0x09
    assert_eq!(vm.cpu.registers.ax.low(), 0x09);
    assert!(vm.cpu.check_flag(Carry));
}
...
fn aam_basic() {
    ...
    vm.cpu.registers.ax.set_low(35); // 35 / 10 = AH=3, AL=5
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.high(), 3);
...
fn aad_basic() {
    ...
    vm.cpu.registers.ax.set_high(3);
    vm.cpu.registers.ax.set_low(5);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 35);
    assert_eq!(vm.cpu.registers.ax.high(), 0);
}
...
fn daa_basic() {
    ...
    vm.cpu.registers.ax.set_low(0x0A);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x10);
    assert!(vm.cpu.check_flag(AuxCarry));
}
...
fn das_basic() {
    ...
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x0A);
}
...
fn and_ax_imm() {
    ...
    vm.cpu.registers.ax.set(0x1234);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x0034);
}
...
fn or_al_imm() {
    ...
    vm.cpu.registers.ax.set_low(0x0A);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0xFA);
    assert!(!vm.cpu.check_flag(Carry));
    assert!(!vm.cpu.check_flag(Overflow));
    assert!(vm.cpu.check_flag(Sign));
}
...
fn xor_al_imm() {
    ...
    vm.cpu.registers.ax.set_low(0xAA);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.low(), 0x55);
}
...
fn not_word() {
    ...
    vm.cpu.registers.ax.set(0xFF00);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x00FF);
}
...
fn test_rm_reg() {
    ...
    vm.cpu.registers.ax.set(0x00FF);
    vm.cpu.registers.bx.set(0xFF00);
    exec(&mut vm);
    assert!(vm.cpu.check_flag(Zero)); // no overlapping bits
}
...
fn shl_by_1() {
    ...
    vm.cpu.registers.ax.set(0x4000);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x8000);
    assert!(!vm.cpu.check_flag(Carry));
}
...
fn shr_by_1() {
    ...
    vm.cpu.registers.ax.set(0x0004);
    exec(&mut vm);
...
fn shr_by_cl() {
    ...
    vm.cpu.registers.ax.set(0x0100);
    vm.cpu.registers.cx.set_low(4);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x0010);
}
...
fn sar_by_1_negative() {
    ...
    vm.cpu.registers.ax.set(0x8000);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0xC000); // sign preserved
}
...
fn rol_by_1() {
    ...
    vm.cpu.registers.ax.set(0x8001);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x0003); // MSB wraps to bit 0
    assert!(vm.cpu.check_flag(Carry)); // CF = bit 0 of result
}
...
fn ror_by_1() {
    ...
    vm.cpu.registers.ax.set(0x0001);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ax.word(), 0x8000); // LSB wraps to MSB
    assert!(vm.cpu.check_flag(Carry)); // CF = MSB of result
}
...
fn rcl_by_1() {
    ...
    assert_eq!(vm.cpu.registers.ax.word(), 0x0001);
    assert!(vm.cpu.check_flag(Carry)); // old MSB was 1
}
...
fn rcr_by_1() {
    ...
    assert_eq!(vm.cpu.registers.ax.word(), 0x8000);
    assert!(vm.cpu.check_flag(Carry)); // old LSB was 1
}
...
fn shl_byte_by_1() {
    ...
    vm.cpu.registers.ax.set_low(0x81);
    exec(&mut vm);
...
fn movsb_single() {
    ...
    vm.cpu.registers.di.set(0x300);
    vm.cpu.registers.ds.write_byte(0x200, 0xAB);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.es.read_byte(0x300), 0xAB);
    assert_eq!(vm.cpu.registers.si.word(), 0x201);
    assert_eq!(vm.cpu.registers.di.word(), 0x301);
}
...
fn movsw_single() {
    ...
    vm.cpu.registers.ds.write_word(0x200, 0xBEEF);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.es.read_word(0x300), 0xBEEF);
    assert_eq!(vm.cpu.registers.si.word(), 0x202);
    assert_eq!(vm.cpu.registers.di.word(), 0x302);
}
...
fn rep_movsb() {
    ...
    vm.cpu.registers.si.set(0x200);
    vm.cpu.registers.di.set(0x300);
    vm.cpu.registers.cx.set(3);
    vm.cpu.registers.ds.write_byte(0x200, 0x11);
    vm.cpu.registers.ds.write_byte(0x201, 0x22);
    vm.cpu.registers.ds.write_byte(0x202, 0x33);
    exec_n(&mut vm, 2); // REP prefix + MOVSB
    assert_eq!(vm.cpu.registers.es.read_byte(0x300), 0x11);
    assert_eq!(vm.cpu.registers.es.read_byte(0x301), 0x22);
    assert_eq!(vm.cpu.registers.es.read_byte(0x302), 0x33);
    assert_eq!(vm.cpu.registers.cx.word(), 0);
}
...
fn stosb_single() {
    ...
    vm.cpu.registers.di.set(0x300);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.es.read_byte(0x300), 0x42);
    assert_eq!(vm.cpu.registers.di.word(), 0x301);
}
...
fn stosw_single() {
    ...
    vm.cpu.registers.ax.set(0xBEEF);
    vm.cpu.registers.di.set(0x300);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.es.read_word(0x300), 0xBEEF);
    assert_eq!(vm.cpu.registers.di.word(), 0x302);
...
fn rep_stosb() {
    ...
    vm.cpu.registers.ax.set_low(0xFF);
    vm.cpu.registers.di.set(0x300);
    vm.cpu.registers.cx.set(4);
    exec_n(&mut vm, 2);
    for i in 0..4u16 {
        assert_eq!(vm.cpu.registers.es.read_byte(0x300 + i), 0xFF);
    }
    assert_eq!(vm.cpu.registers.cx.word(), 0);
}
...
fn lodsw_single() {
    ...
    vm.cpu.registers.si.set(0x200);
    vm.cpu.registers.ds.write_word(0x200, 0xCAFE);
    exec(&mut vm);
...
fn scasb_equal() {
    ...
    vm.cpu.registers.ax.set_low(0x42);
    vm.cpu.registers.di.set(0x300);
    vm.cpu.registers.es.write_byte(0x300, 0x42);
    exec(&mut vm);
    assert!(vm.cpu.check_flag(Zero));
    assert_eq!(vm.cpu.registers.di.word(), 0x301);
}
...
fn scasb_not_equal() {
    ...
    vm.cpu.registers.ax.set_low(0x42);
    vm.cpu.registers.di.set(0x300);
    vm.cpu.registers.es.write_byte(0x300, 0x43);
    exec(&mut vm);
    assert!(!vm.cpu.check_flag(Zero));
}
...
fn cmpsb_equal() {
    ...
    vm.cpu.registers.si.set(0x200);
    vm.cpu.registers.di.set(0x300);
    vm.cpu.registers.ds.write_byte(0x200, 0x55);
    vm.cpu.registers.es.write_byte(0x300, 0x55);
    exec(&mut vm);
    assert!(vm.cpu.check_flag(Zero));
}
...
fn cmpsb_not_equal() {
    ...
    vm.cpu.registers.di.set(0x300);
    vm.cpu.registers.ds.write_byte(0x200, 0x55);
    vm.cpu.registers.es.write_byte(0x300, 0x56);
    exec(&mut vm);
    assert!(!vm.cpu.check_flag(Zero));
}
...
fn movsb_direction_flag() {
    ...
    vm.cpu.registers.si.set(0x205);
    vm.cpu.registers.di.set(0x305);
    vm.cpu.registers.ds.write_byte(0x205, 0xEE);
    exec_n(&mut vm, 2);
    assert_eq!(vm.cpu.registers.es.read_byte(0x305), 0xEE);
    assert_eq!(vm.cpu.registers.si.word(), 0x204);
    assert_eq!(vm.cpu.registers.di.word(), 0x304);
}
...
fn repe_cmpsb() {
    ...
    for i in 0..3u16 {
        vm.cpu.registers.ds.write_byte(0x200 + i, 0xAA);
        vm.cpu.registers.es.write_byte(0x300 + i, 0xAA);
    }
    vm.cpu.registers.ds.write_byte(0x203, 0xAA);
    vm.cpu.registers.es.write_byte(0x303, 0xBB);
    exec_n(&mut vm, 2);
    assert!(!vm.cpu.check_flag(Zero)); // mismatch found
    assert_eq!(vm.cpu.registers.cx.word(), 0); // all compared
}
...
fn repne_scasb() {
    ...
    vm.cpu.registers.ax.set_low(0x42);
    vm.cpu.registers.di.set(0x300);
    vm.cpu.registers.cx.set(5);
    vm.cpu.registers.es.write_byte(0x300, 0x00);
    vm.cpu.registers.es.write_byte(0x301, 0x00);
    vm.cpu.registers.es.write_byte(0x302, 0x42); // match at [302]
    vm.cpu.registers.es.write_byte(0x303, 0x00);
    exec_n(&mut vm, 2);
    assert!(vm.cpu.check_flag(Zero)); // found
    assert_eq!(vm.cpu.registers.cx.word(), 2); // 5-3=2
}
...
fn jmp_short() {
    ...
    assert_eq!(vm.cpu.registers.ax.low(), 0x42);
    assert_eq!(vm.cpu.registers.pc.word(), 6);
}
...
fn jmp_far() {
    ...
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.pc.word(), 0x0000);
    assert_eq!(vm.cpu.registers.cs.reg().word(), 0x1000);
}
...
fn call_far_retf() {
    ...
    Memory::copy_data(&mut vm.cpu.memory, 0x0010, &[0xB0, 0x42, 0xCB]);
    exec(&mut vm); // CALL far
    assert_eq!(vm.cpu.registers.pc.word(), 0x0010);
    exec(&mut vm); // MOV AL,0x42
    assert_eq!(vm.cpu.registers.ax.low(), 0x42);
    exec(&mut vm); // RETF
    assert_eq!(vm.cpu.registers.pc.word(), 0x0005); // after the CALL instruction
}
...
fn ret_imm() {
    ...
    let mut vm = setup(&[]);
    Memory::copy_data(&mut vm.cpu.memory, 0x100, &[0xC2, 0x04, 0x00]);
    vm.cpu.registers.pc.set(0x100);
    vm.cpu.registers.sp.set(0x8000);
    vm.push_word(0x1234); // push return address, SP=0x7FFE
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.pc.word(), 0x1234);
    assert_eq!(vm.cpu.registers.sp.word(), 0x8000 + 4); // original SP + 4 extra
}
...
fn retf_imm() {
    ...
    let mut vm = setup(&[]);
    Memory::copy_data(&mut vm.cpu.memory, 0x100, &[0xCA, 0x02, 0x00]);
    vm.cpu.registers.pc.set(0x100);
    vm.cpu.registers.sp.set(0x8000);
    vm.push_word(0x0000); // push CS (must be 0 so fetch works after)
    vm.push_word(0x0050); // push IP
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.pc.word(), 0x0050);
    assert_eq!(vm.cpu.registers.cs.reg().word(), 0x0000);
    assert_eq!(vm.cpu.registers.sp.word(), 0x8000 + 2); // original SP + 2 extra
}
...
fn cld_std() {
    ...
    exec(&mut vm);
    assert!(vm.cpu.check_flag(Directional));
    exec(&mut vm);
    assert!(!vm.cpu.check_flag(Directional));
}
...
fn cli_sti() {
    ...
    exec(&mut vm); // CLI
    assert!(!vm.cpu.check_flag(Interrupt));
    exec(&mut vm); // STI
    assert!(vm.cpu.check_flag(Interrupt));
}
...
fn nop() {
    ...
    let pc_before = vm.cpu.registers.pc.word();
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.pc.word(), pc_before + 1);
}
...
fn push_rm_word() {
    ...
    vm.cpu.registers.ax.set(0xBEEF);
    let old_sp = vm.cpu.registers.sp.word();
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.sp.word(), old_sp - 2);
    assert_eq!(vm.cpu.registers.ss.read_word(vm.cpu.registers.sp.word()), 0xBEEF);
}
...
fn inc_rm_byte() {
    ...
    vm.cpu.registers.ds.write_byte(0x200, 0x41);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ds.read_byte(0x200), 0x42);
}
...
fn dec_rm_byte() {
    ...
    vm.cpu.registers.ds.write_byte(0x200, 0x42);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ds.read_byte(0x200), 0x41);
}
...
fn inc_rm_word() {
    ...
    vm.cpu.registers.ds.write_word(0x200, 0x00FF);
    exec(&mut vm);
    assert_eq!(vm.cpu.registers.ds.read_word(0x200), 0x0100);
}
...
fn dec_rm_word() {
    ...
    vm.cpu.registers.ds.write_word(0x200, 0x0100);
    exec(&mut vm);
